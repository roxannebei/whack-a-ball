...
The following code is for the <ball>/<weighted ball> condition:

For the <mouse> condition: see another .pde file
...
// for hammer, hit, hole, mole_0, mole_1
...
void draw() {
  ...
  if (!mousePressed) {
    ...
    image(loadImage("hammer.png"), handX-50, handY-50, 103, 127); //size 310*383
  } else {
    //image(loadImage("hit.png"), mouseX-50, mouseY-50, 125, 103); //size 377*310
    image(loadImage("hit.png"), handX-50, handY-50, 125, 103); //size 377*310
  }
  ...
  // begin the loop to traverse every pixel
  for (int x = 0; x < video.width; x ++ ) {
    for (int y = 0; y < video.height; y ++ ) {
      int loc = x + y*video.width;
      // check what is the current color
      ...
      float d = dist(r1, g1, b1, r2, g2, b2); // the dist( ) function is used to compare the current color with the color we are tracking
  ...
  if (count > 5) { 
    ...
    strokeWeight(10);
    fill(255, 0, 0);
    ellipse(width-avgX*2.5, avgY*1.875, 20, 20);
    handX = width-avgX*2.5;
    handY = avgY*1.875;
  }
...
class Mole {
  ...
  boolean hit() {  
    ...
    if (handX>x+50 && handX<x+195 && handY>y-30 && handY<y+110) {
      hit=true;
      punch.play();
      ouch.play();
      return true;
